...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM characters JOIN series ON series.id = characters.series_id WHERE characters.species='human' GROUP BY characters.species, characters.series_id ORDER BY COUNT(characters.species) DESC LIMIT 1;"
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(characters.name) AS book_count FROM character_books JOIN characters ON characters.id = character_books.character_id GROUP BY characters.name ORDER BY book_count DESC;"
end
